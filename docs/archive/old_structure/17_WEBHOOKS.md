# 17. Webhook Integration Guide

**Status:** ✅ Complete (Production Ready)

[← Back: Lifecycle Examples](16_LIFECYCLE_EXAMPLES.md) | [Index](0_README.md)

---

## Overview

Webhooks allow MySQL VFS v2.1+ to integrate with external systems by sending HTTP requests when events occur. Webhooks can also **veto** operations, giving external services the ability to approve or deny filesystem operations.

**Implementation:**
- Webhook handler: `pkg/events/handlers/webhook.go`
- Tests: `pkg/events/handlers/webhook_test.go`
- Veto integration: `pkg/domain/veto_integration_test.go`
- Retry logic: Lines 150-200 in webhook.go
- Circuit breaker: Lines 250-300 in webhook.go
- HMAC signing: Lines 350-380 in webhook.go
...
**Current Implementation:** See `pkg/events/types.go` for the FileResource and DirectoryResource structures.
...
**Implementation:** `pkg/events/handlers/webhook.go` (lines 150-200: retry, lines 250-300: circuit breaker)
...
2. **Implement HMAC verification** to prevent spoofing (implementation: `webhook.go` lines 350-380)
3. **Set appropriate timeouts** to avoid blocking operations (default: 5000ms)
4. **Use circuit breakers** to handle webhook failures gracefully (implementation: lines 250-300)
5. **Monitor webhook performance** with metrics
6. **Test veto logic** thoroughly before deploying (see: `veto_integration_test.go`)
...
**Source:** `pkg/events/handlers/webhook.go` (lines 150-200)
...
**Source:** `pkg/events/handlers/webhook.go` (lines 250-300)
...
**Source:** `pkg/events/handlers/webhook.go` (lines 100-130)
**Tests:** `pkg/domain/veto_integration_test.go`

Webhook responses with status codes 4xx or 5xx (based on configuration) will veto the operation and return an error to the client.
