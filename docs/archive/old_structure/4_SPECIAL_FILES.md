...
2. Validate content:
   ...
   ↓
3. Trigger events:
   - EventsLoader.GetHandlersForEvent(dirID, "file.create.completion.succeeded")
...
```json
{
  "handlers": [
    ...
    {
      "name": "log-all",
      "events": ["file.{create,delete}.completion.succeeded"],
      "type": "log",
      "enabled": true,
      "config": {
        "level": "info"
...
curl -X POST http://localhost:8080/api/v1/files \
  ...
  -d '{
    "directory_path": "/data",
    "name": ".events",
    "content": "{\"handlers\":[{\"name\":\"webhook\",\"events\":[\"file.create.completion.succeeded\"],\"type\":\"webhook\",\"config\":{\"url\":\"https://example.com/hook\"}}]}"
  }'
```
