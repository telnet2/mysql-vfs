...
- Authentication system changes (group-based)
...
**Changes:**
- Uses `Groups` array in `AuthContext` (<pkg>middleware.auth.go</pkg>)
- Uses `Groups` array in `UserCredential` (<pkg>domain.special_files.go</pkg>)
- System admin group (`system-admin`) bypasses ALL rego authorization
- Separated `admin` group (follows rego) from `system-admin` group (bypasses rego)
...
**Impact:**
- Flexible group-based authorization
- Clear separation of admin types
- Users can belong to multiple groups
- Authorization via OPA policies checking `input.user.groups[_]`
...
- Default policies: `pkg/middleware/default_policy.go`, `pkg/setup/setup.go`
...
- Default group changed from `"admin"` to `"system-admin"`
- Centralized admin checking via group membership in `"system-admin"`
...
```
Operation Flow:
1. Authorization  → Policy check → Permission check → Group check
