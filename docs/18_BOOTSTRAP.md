...
- `user` role → read-only access
- `user` role + owner group → read+write+delete access in owned directories
...
curl -X POST http://localhost:8080/api/v1/files \
  ...
  -d '{
    ...
    "name": ".user",
    "content_type": "application/json",
    "content": "{\"users\": [{\"user_id\": \"alice\", \"password_hash\": \"$2a$10$...\", \"role\": \"admin\"}]}"
  }'
```
...
| `user` | Read-only (unless owner) | `pkg/setup/setup.go` (line 23) |

### Ownership

Users with role `user` can write/delete in directories they own:
- Ownership is defined in `.owner` files (<pkg>domain.owner_loader.go</pkg>)
- User must match the owner field
...
**Example 1: Admin has full access**
```
User: alice (role: admin)
Action: write to /data/file.json
Result: ✅ Allowed (admin rule at line 18)
```

**Example 2: User can read**
```
User: bob (role: user)
Action: read /data/file.json
Result: ✅ Allowed (user read rule at line 23)
```
...
Directory owner: charlie
Result: ✅ Allowed (owner rule at line 28)
```
